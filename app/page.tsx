...
const WATCH_SORTS: Array<{ key: WatchSort; label: string }> = [
  { key: "risk", label: "风险优先" },
  { key: "latest", label: "最新同步" },
  { key: "sales", label: "销量最高" },
  { key: "price", label: "价格最低" },
  { key: "traffic", label: "广告贡献" },
];
...
function numericValue(value: number | null | undefined, fallback = -Infinity) {
  return typeof value === "number" && Number.isFinite(value) ? value : fallback;
}

function hasActivePromotion(result: AnalysisResult) {
...
function issueTags(result: AnalysisResult, targetState?: MonitorTargetState | null) {
  const tags: string[] = [];
  const addPercentTag = (change: MetricChange, label: string, upLabel = `${label}上涨`, downLabel = `${label}下降`) => {
    if (change.previous !== null && change.percent !== null && Math.abs(change.percent) >= 15) {
      tags.push(change.percent > 0 ? upLabel : downLabel);
    }
  };

  if (targetState?.lastStatus === "failed") tags.push("同步失败");
  addPercentTag(result.changes.effectivePrice, "价格");
  addPercentTag(result.changes.monthlyUnits, "销量", "销量增长", "销量下降");
...
function Bars({ points, values, labels, formatter }: { points: string[]; values: Array<number | null>; labels: string[]; formatter: (value: number | null) => string }) {
  ...
  const plotted = values.map((value, index) => {
    ...
    const y = range === 0 ? 50 : 88 - ((value - min) / range) * 76;
    return { x, y, value, label: labels[index], point: points[index] };
  });
  const linePoints = plotted.filter((item): item is NonNullable<typeof item> => item !== null);
  const axisLabels = values.length <= 3
    ? labels
    : [labels[0], labels[Math.floor(labels.length / 2)], labels[labels.length - 1]];

  return (
    <div className="line-chart" aria-label={labels.join(" ")}>
      <div className="line-chart-range"><span>{formatter(min)}</span><span>{formatter(max)}</span></div>
      <svg className="line-plot" viewBox="0 0 100 100" preserveAspectRatio="none" role="img">
        <line x1="0" y1="12" x2="100" y2="12" />
        <line x1="0" y1="50" x2="100" y2="50" />
        <line x1="0" y1="88" x2="100" y2="88" />
        {linePoints.length > 1 && <polyline points={linePoints.map((item) => `${item.x},${item.y}`).join(" ")} />}
        {linePoints.map((item) => (
          <circle cx={item.x} cy={item.y} r="1.8" key={`${item.point}:${item.value}`}>
            <title>{item.label} · {formatter(item.value)}</title>
          </circle>
        ))}
      </svg>
      <div className="line-axis">{axisLabels.map((label, index) => <time key={`${label}:${index}`}>{label}</time>)}</div>
    </div>
  );
}
...
function decisionSummary(result: AnalysisResult) {
  ...
  if (material.changed) {
    return {
      ...
      title: `${formatMaterialSignal(material.top)}，需要复核`,
      body: first?.body ?? "该竞品出现显著波动，建议结合促销、BSR、销量和广告位一起判断。",
      action,
      confidence,
      tags,
    };
  ...
  if (result.history.length <= 1) {
    return {
      tone: "info" as const,
      title: "已建立今日监控基线",
      body: "这是首个同源快照，下一自然日同步后才会形成价格、销量、评论、流量和 Listing 的日环比。",
      action,
      confidence,
      tags,
    };
  ...
  return {
    ...
    title: "今日暂无显著异常",
    body: first?.body ?? "当前价格、销量、评论、促销、流量和 Listing 变化未达到监控阈值。",
    action,
    confidence,
    tags,
  };
...
function SnapshotEvents({ result }: { result: AnalysisResult }) {
  const events = [
    ...(result.promotionChanges.changed
      ? result.promotionChanges.summaries.map((summary) => ({ tone: "warning", label: "促销", body: summary }))
      : []),
    ...(result.listingChanges.changed
      ? result.listingChanges.summaries.map((summary) => ({ tone: "warning", label: "Listing", body: summary }))
      : []),
  ...
  if (!events.length) {
    events.push({
      tone: "baseline",
      label: result.history.length <= 1 ? "基线" : "稳定",
      body: result.history.length <= 1 ? "等待下一自然日形成日环比" : "近期待监控项未触发事件",
    });
  }

  return (
    <div className="event-strip" aria-label="关键事件时间轴">
      {events.slice(0, 6).map((event, index) => (
        <span className={`event-chip ${event.tone}`} key={`${event.label}:${event.body}:${index}`}>
...
function SnapshotDetail({ result }: { result: AnalysisResult }) {
  ...
  const decision = decisionSummary(result);
  return (
    <article className="product-card detail-card">
      ...
      <section className={`decision-panel ${decision.tone}`}>
        <div className="decision-main">
          <span className="section-label">Today Brief</span>
          <h3>{decision.title}</h3>
          <p>{decision.body}</p>
          <div className="issue-tags">{decision.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
        </div>
...
export default function Home() {
  ...
  const [watchSort, setWatchSort] = useState<WatchSort>("risk");
  ...
  useEffect(() => {
    ...
    fetch("/api/analyze").then(async (response) => ({ ok: response.ok, payload: (await response.json()) as MonitorResponse & { error?: string } })).then(({ ok, payload }) => {
      ...
      if (!ok) {
        ...
        return setMessage(friendlyError(payload.error, "监控记录暂时无法读取"));
      }
      ...
      if (!active) return;
      setServiceState("degraded");
      setMessage(friendlyError("历史记录暂时无法读取，可直接同步今日数据"));
    });
    return () => { active = false; };
  }, []);
  ...
  const rankedResults = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      const stateA = targetStateMap.get(`${a.marketplace}:${a.asin}`);
      const stateB = targetStateMap.get(`${b.marketplace}:${b.asin}`);
      if (watchSort === "latest") return Date.parse(b.capturedAt) - Date.parse(a.capturedAt);
      if (watchSort === "sales") return numericValue(b.metrics.monthlyUnits) - numericValue(a.metrics.monthlyUnits);
      if (watchSort === "price") return numericValue(a.metrics.effectivePrice, Infinity) - numericValue(b.metrics.effectivePrice, Infinity);
      if (watchSort === "traffic") return numericValue(b.traffic.adTrafficShare) - numericValue(a.traffic.adTrafficShare);
  ...
  async function handleSync(event: FormEvent) {
    ...
    try {
      ...
      const failedText = payload.failures?.length ? `；失败：${payload.failures.slice(0, 3).map((item) => `${item.marketplace} ${item.asin}（${friendlyError(item.error)}）`).join("、")}` : "";
      setMessage(payload.persisted ? `今日快照已保存，已加入每日自动同步${failedText}` : "同步完成，但历史库暂时不可写");
    } catch (error) {
      setMessage(friendlyError(error, "同步失败，请稍后重试"));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleAutoSync(item: AnalysisResult) {
    ...
    try {
      ...
      setMessage(friendlyError(error, "自动同步设置失败"));
    } finally {
      setUpdatingAutoKey("");
    }
  }
  ...
  async function handleHistoryQuery(event: FormEvent) {
    ...
    try {
      ...
      setHistoryMessage(friendlyError(error, "历史查询失败"));
    } finally {
  ...
  async function handleDelete(item: AnalysisResult) {
    ...
    try {
      ...
      setMessage(friendlyError(error, "删除监控失败，请稍后重试"));
    } finally {
      setDeletingKey("");
    }
  }
  ...
  const platformPriceChange = latestPlatformPrice !== null && firstPlatformPrice !== null && firstPlatformPrice !== 0
    ...
    : null;
  const messageExplanation = explainUserError(message);
  const messageTone = userErrorTone(message);
  const historyTone = userErrorTone(historyMessage);
          ...
          <div className="page-content">
            <section className="welcome-row">
              ...
              <form className="sync-card" onSubmit={handleSync}>
                ...
                <p className={`status-line ${messageTone}`} role="status">{message}</p>
              </form>
            </section>
            {serviceState === "degraded" && (
              <section className={`status-banner ${messageExplanation.tone}`}>
                <strong>{messageExplanation.title}</strong>
                <span>{messageExplanation.detail}</span>
              </section>
            )}
            ...
            <section className="product-card watchlist-card">
              <div className="card-heading"><div><span className="section-label">Watchlist</span><h2>监控列表</h2></div><span>{rankedResults.length} / {results.length} 个商品</span></div>
              <div className="watch-toolbar" aria-label="监控列表筛选">
                <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="搜索 ASIN / 标题 / 品牌" aria-label="搜索监控商品" />
                <select value={marketplaceFilter} onChange={(event) => setMarketplaceFilter(event.target.value)} aria-label="筛选站点">
                  <option value="ALL">全部站点</option>
                  {MARKETPLACES.map((item) => <option key={item} value={item}>{item}</option>)}
                </select>
                <select value={watchFilter} onChange={(event) => setWatchFilter(event.target.value as WatchFilter)} aria-label="筛选状态">
                  {WATCH_FILTERS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
                <select value={watchSort} onChange={(event) => setWatchSort(event.target.value as WatchSort)} aria-label="排序方式">
                  {WATCH_SORTS.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <div className="table-scroll">
          ...
          <div className="page-content history-page">
            <section className="history-hero">
              ...
              <form className="history-search" onSubmit={handleHistoryQuery}>
                <label>查询对象</label><div className="history-fields"><select aria-label="历史查询站点" value={historyMarketplace} onChange={(event) => setHistoryMarketplace(event.target.value)}>{MARKETPLACES.map((item) => <option key={item}>{item}</option>)}</select><input value={historyAsin} onChange={(event) => setHistoryAsin(event.target.value.toUpperCase())} placeholder="输入 10 位 ASIN" maxLength={10} /><select aria-label="历史范围" value={historyDays} onChange={(event) => setHistoryDays(event.target.value)}><option value="30">近 30 天</option><option value="90">近 90 天</option><option value="180">近 180 天</option><option value="365">近 365 天</option></select><button type="submit" disabled={historyLoading}>{historyLoading ? "查询中…" : "查询历史"}</button></div><p className={`status-line ${historyTone}`} role="status">{historyMessage}</p>
              </form>
            </section>
